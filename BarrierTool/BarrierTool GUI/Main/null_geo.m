function varargout = null_geo(varargin)
% NULL_GEO MATLAB code for null_geo.fig
%      NULL_GEO, by itself, creates a new NULL_GEO or raises the existing
%      singleton*.
%
%      H = NULL_GEO returns the handle to a new NULL_GEO or the handle to
%      the existing singleton*.
%
%      NULL_GEO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NULL_GEO.M with the given input arguments.
%
%      NULL_GEO('Property','Value',...) creates a new NULL_GEO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before null_geo_Fcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to null_geo_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help null_geo

% Last Modified by GUIDE v2.5 07-Jul-2018 17:06:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @null_geo_OpeningFcn, ...
                   'gui_OutputFcn',  @null_geo_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before null_geo is made visible.
function null_geo_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to null_geo (see VARARGIN)
...
% Choose default command line output for null_geo
...
% UIWAIT makes null_geo wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = null_geo_OutputFcn(hObject, eventdata, handles) 
...
%% Step1: Compute r_\lambda(0)=(x_0,\phi0) (cf. eq. (39) of [1])                                          
 ...
 %% Step2: Compute \phi'
    ...
    [C22mC11Gr,C12Gr,phiPrGr]=Phi_prime(handles.C11,handles.C11x1,handles.C11x2,handles.C12,handles.C12x1,handles.C12x2,handles.C22,handles.C22x1,handles.C22x2,handles.x1_g,handles.x2_g,handles.interpolationIn2D);
 ...
 %% Step3: Solve the Initial value null-geodesic problem (cf. eqs. (38-39) of [1])
    ...
    [handles.x1Psol,handles.x2Psol] = FindClosedNullGeod(C22mC11Gr,C12Gr,phiPrGr,x0lam,y0lam,phi0lam,handles.x1_g,handles.x2_g,handles.lamV,handles.sVec,handles.NCores,handles.b_h,handles.interpolationIn2D);
